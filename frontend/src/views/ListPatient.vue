<template>
  <div class="min-h-screen">
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm mb-4">
      <div class="flex flex-wrap gap-3 mb-3">
        <span class="p-input-icon-left flex-1">
          <InputText v-model="filters.global.value" placeholder="Buscar paciente..." class="w-full" />
        </span>
        <Dropdown 
    ...
    <div class="bg-white rounded-lg shadow-sm">
        ...
        <Column field="name" header="Nombre" sortable>
          <template #body="{ data }">
            <div class="flex items-center">
              <Avatar :label="getInitials(data.name)" shape="circle" size="large" class="mr-2" />
              <div>{{ data.name }}</div>
            </div>
          </template>
        </Column>
      ...
      <!-- Batch actions -->
      <div v-if="selectedPatients.length > 0" class="p-3 bg-gray-100 border-t flex justify-between items-center">
        <span class="text-sm text-gray-700">{{ selectedPatients.length }} pacientes seleccionados</span>
        <div>
          <Button 
            label="Enviar Correo" 
            icon="pi pi-envelope" 
    ...
    >
      <div v-if="selectedPatient" class="grid grid-cols-1 gap-4">
        <div class="flex justify-center mb-3">
          <Avatar :label="getInitials(selectedPatient.name)" size="xlarge" shape="circle" />
        </div>
        <div class="text-center mb-3">
          <h2 class="text-xl font-bold">{{ selectedPatient.name }}</h2>
          <Tag :value="selectedPatient.isapre.value" :severity="getIsapreSeverity(selectedPatient.isapre.value)" class="mt-2" />
        </div>
      ...
      <template #footer>
        <Button label="Cerrar" icon="pi pi-times" @click="patientDetailsDialog = false" class="p-button-text" />
        <Button label="Editar" icon="pi pi-pencil" @click="editFromDetails" class="p-button-primary" />
      </template>
    </Dialog>
    ...
    >
      <div class="flex align-items-center justify-content-center">
        <i class="pi pi-exclamation-triangle mr-3 text-3xl text-yellow-500" />
        <span>
          ¿Está seguro de que desea eliminar {{ selectedPatients.length }} pacientes?<br>
          <small class="text-gray-500">Esta acción no se puede deshacer.</small>
        </span>
      </div>
      <template #footer>
...
const editFromDetails = () => {
  if (selectedPatient.value) {
    ...
    router.push({ name: "Crear paciente", params: { id: selectedPatient.value.id } });
  }
};
...
const deleteMultipleUsers = async () => {
  try {
    ...
    const response = await apiClient.post("/patients/delete-multiple", { ids: patientIds });
    ...
    toast.add({
      severity: "error",
      summary: "Error",
      detail: error.response?.data?.message || "No se pudieron eliminar los pacientes",
      life: 3000,
    });
  } finally {
